...
export const UpdateROI = ({ item, onCancel, actionRef, show }: { item:IMerchandiseItem|null, onCancel: () => void, actionRef: any, show: boolean }) => {
    ...
    return (
        ...
        >
            {Boolean(item.targetRoi) && (
                <>
                    <div>目标投产比 <InputNumber size='small' defaultValue={item.targetRoi} onChange={v => setInputData(v as number)} /></div>
            ...
            {Boolean(item.optimizationBid)&& (
                <>
                    <div>成交出价 <InputNumber size='small' defaultValue={item.optimizationBid} onChange={v => setInputData(v as number)} /> 元 成交</div>
