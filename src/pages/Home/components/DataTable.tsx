import { IAllMerchandiseParams, IMerchandiseItem, MerchandiseType, getAllMerchandise, updateGoodsStatus } from '@/api';
...
import { Button, DatePicker, Dropdown, MenuProps, Popover, Switch, message } from 'antd';
...
export default () => {
  ...
  const columns: ProColumns<IMerchandiseItem>[] = [
    ...
    },
    {
      ...
      render: (_, item) => {
          ...
          if (item.optimizationBid) return (
            <div>
          ...
          if (item.targetRoi) return (
            <div> { `目标投产比：${item.targetRoi}`} <FormOutlined onClick={openit} /></div>
          )
          return '-'
      }
    },
  ...
  return (
    <>
      <CreateTable isOpen={createShow} onCancel={() => setCreateShow(false)} actionRef={actionRef} />
      <UpdateROI show={showROIModal} onCancel={setShowROIMdal.bind(null, false)} item={selectedItem} actionRef={actionRef} />
      <UpdateMaxCost show={showCostMdal} actionRef={actionRef} item={selectedItem} onCancel={setShowCostModal.bind(null, false)} />
