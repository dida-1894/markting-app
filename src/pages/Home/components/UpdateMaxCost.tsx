...
export const UpdateMaxCost = ({ item, onCancel, actionRef, show }: { item:IMerchandiseItem|null, onCancel: () => void, actionRef: any, show: boolean }) => {
    ...
    return (
        <Modal
            ...
            onOk={async () => {
                ...
                onCancel()
                const { code } = await updateMaxCost(item.goodsId, radioStatus === NoLimitMaxCost ? NoLimitMaxCost : inputData as number)
                if (code === 200) {
        ...
        >
            ...
            >
                ...
                <div>
                    <Radio value={CustomMaxCost}>
                        自定义
                    </Radio>
                    <InputNumber status={status} disabled={radioStatus === NoLimitMaxCost} size='small' value={inputData} onChange={v => setInputData(v as number)} />
                </div>
            </Radio.Group>
            {status === 'error' && <div style={{ color: 'red' }}>修改日限额只能介于{MinMaxCost}~{NoLimitMaxCost}</div>}
